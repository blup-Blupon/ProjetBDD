...
def showCourses():
    with Model() as model:
        ...
        keys = [
            '', 'Name', '', 'Teacher Firstname', 'Teacher Lastname', 'Details',
            'Delete'
        ]
        return render_template(
            'listing.html',
...
def showCourse(id=None):
    with Model() as model:
        ...
        if request.method == 'POST' and form.validate():
            model.addValidationToCourse(form.name.data, form.coef.data, id)
...
def showRoom(id=None):
    with Model() as model:
        ...
        form.setNames()
        if request.method == 'POST' and form.validate():
            model.occupyRoom(id, form.course.data, form.start.data,
                             form.end.data)
        ...
        return render_template(
            'listing.html',
            to_list=[(exams, keys,
                      "Usage of room " + model.getNameOfRoom(id))],
            forms=[form])
